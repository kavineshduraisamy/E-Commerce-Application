 ...
 const cartIds = [];
  ...
  const totalHandler = () => {
    return Orders.map(data =>{
      return (price = Math.floor(data.Product.Rate - (data.Product.Rate / 100) * data.Product.offer) * data.quantity + price - discount);
    });
  };
    ...
    <Card>
      <CardBody>
        ...
        <Row className="cartSidePriceRow">
          ...
          <Col className="cartSidePrice">{totalHandler()-discountHandler()} </Col>
